...
<script type="text/JavaScript">
	...
	function populateTermReg(batchId) {
		$('#subTable tbody').empty();
		
		//request the JSON data and parse into the select element
		$.ajax({
			type:"GET",
			url:"${pageContext.request.contextPath}/viewAllReg?tenantId="+batchId,
			contentType:"application/json; charset=utf-8",
			dataType:"json",
			success:function(data){
				$('#subTable tbody').empty();
				$.each(data,function(i,item){
					    var url1 = "${pageContext.request.contextPath}/viewRegDetail?batchName="+batchId+"&termCode="+data[i].termCode+"&id="+data[i].id+"&status="+data[i].status ;
					    url1 = encodeURI(url1);
						$('#subTable tbody').append('<tr><td>'+data[i].id+'</td><td>'+ data[i].batchName +'</td><td>'+ data[i].termCode +'</td><td>'+ data[i].status+ '</td><td><a class="btn btn-info btn-sm" href='+url1+'>Update</a></td></tr>');
				});
			},
			complete :function() {
